...
function getPackageVersion(packageName, controlRoot, logger) {
    ...
    var pathToPackageLockJson = path.resolve(controlRoot, constants.PACKAGE_LOCK_JSON_PATH);
    ...
    var pathToPackageJson = path.resolve(controlRoot, constants.NODE_MODULES, packageName, constants.PACKAGE_JSON_PATH);
...
function tryGetPackageVersionFromPnpApi(controlRoot, packageName) {
    ...
    var locator = pnpApi.findPackageLocator(controlRoot);
    var packageInfo = pnpApi.getPackageInformation(locator);
    // Get strong type from https://yarnpkg.com/advanced/pnpapi#packageinformation
    var dependencies = packageInfo.packageDependencies;
    var dependency = dependencies.get(packageName);
...
function tryReadPackageVersionFromPackageFile(packageFilePath) {
    if (fs.existsSync(packageFilePath)) {
        var parsedPackage = require(packageFilePath);
        var version = parsedPackage === null || parsedPackage === void 0 ? void 0 : parsedPackage.version;
...
function tryReadPackageVersionFromPackageLockFile(packageLockFilePath, packageName, logger) {
    ...
    if (fs.existsSync(packageLockFilePath)) {
        var parsedLock = require(packageLockFilePath);
        var dependencies = parsedLock === null || parsedLock === void 0 ? void 0 : parsedLock.dependencies;
        if (dependencies) {
            var version = (_a = dependencies[packageName]) === null || _a === void 0 ? void 0 : _a.version;
            if (typeof version === "string") {
                ...
                if (version.startsWith("file:")) {
                    ...
                    var pathPart = decodeURI(version).substring(5);
                    var refPackagePath = path.resolve(path.dirname(packageLockFilePath), pathPart, constants.PACKAGE_JSON_PATH);
