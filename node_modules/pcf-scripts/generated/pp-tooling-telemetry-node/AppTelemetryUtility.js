...
function isSupportedAgentProductVersion(productVersion) {
    if (!!productVersion) {
        ...
        let idxPreRelease = productVersion.indexOf('-');
        let idxMetadata = productVersion.indexOf('+');
        let idxPreReleaseOrMetadata = idxPreRelease < 0 ? idxMetadata
            : idxMetadata < 0 ? idxPreRelease
                : Math.min(idxPreRelease, idxMetadata);
        let versionSubStr = idxPreReleaseOrMetadata === -1 ? productVersion : productVersion.substring(0, idxPreReleaseOrMetadata);
...
function tryParseAutomationAgentStringProductNameVersion(agent) {
    ...
    let firstProduct = agent.split(' ', 2)[0];
...
function validateAndSanitizeAutomationAgent(environment) {
    ...
    let result = tryParseAutomationAgentStringProductNameVersion(environment.automationAgent);
...
function createCommonAppStartProperties(environment) {
    let properties = {};
    let agent = validateAndSanitizeAutomationAgent(environment);
    if (!agent) {
